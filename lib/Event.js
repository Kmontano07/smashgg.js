...
class Event extends EventEmitter{
    ...
    static get(url, expands, isCached){
        return new Promise(function(resolve, reject){
            try{
                let E = new Event(url, expands, isCached);
                ...
                E.load()
                    .then(function(){
                        let cacheKey = format(EVENT_TOURNAMENT_CACHE_KEY, E.tournamentName, E.eventName, E.expandsString);
    ...
    constructor(url, expands, isCached){
        super();
        ...
        if(this.url && !regex.test(this.url)) {
            ...
            this.url = this.url + '?' + this.expandsString;
        }
    }

    formatExpands(expands){
        let expandsString = "";
        let newExpands = {
            phase: (expands && expands.phase == false) ? false : true,
            groups: (expands && expands.groups == false) ? false : true
        };
        for(let property in newExpands){
            if(newExpands[property])
                expandsString += format('expand[]=%s&', property);
        }

        return {
            expands: newExpands,
            expandsString: expandsString
        }
    }
    ...
    async load(){
        ...
        try{
            if(!this.url)
                throw new Error('URL property cannot be null for Event');
            ...
            let cacheKey = this.id ?
                ...
                format(EVENT_TOURNAMENT_CACHE_KEY + '::data', this.tournamentName, this.eventName, this.expandsString);
            let cached = await Cache.get(cacheKey);

            if(!cached){
                this.url = this.url.formatHTTPS(); //from StringUtils
